...
module CFramework.CBetaContraction (C : Set) where
  ...
  antipreser▹β* : AntiPreserves* _▹β_
  antipreser▹β* {x} .{ƛ y M · N} x*M[y/N] (beta {y} {M} {N}) with lemmafreeσ→ {x} {M} x*M[y/N]
  ...  | z , z*M , x*z[y/N] with y ≟ z
  antipreser▹β* {x} .{ƛ y M · N} x*M[y/N] (beta {y} {M} {N}) | .y , y*M , x*N | yes refl = *·r x*N
  antipreser▹β* {x} .{ƛ y M · N} x*M[y/N] (beta {y} {M} {N}) | .x , x*M , *v | no y≢z = *·l (*ƛ x*M y≢z)

  preser▹β# : Preserves# _▹β_
  preser▹β# = dual-*-# antipreser▹β*
  ...
  substFresh-aux : ∀ {x M N} → x # M → (y : V) → y * M → ι y ≡ (ι ≺+ (x , N)) y
  substFresh-aux {x} x#M y y*M with x ≟ y
  substFresh-aux {x} x#M .x x*M | yes refl = ⊥-elim (lemma-free→¬# x*M x#M)
  ... | no _ = refl

  substFresh : ∀ {x M N} → x # M → M [ N / x ] ≡ M ∙ ι
  substFresh x#M = sym (lemma-subst-σ≡ (∼*ρ , substFresh-aux x#M))

  redexFresh : ∀ {x M N} → x # M → (ƛ x M · N) ▹β (M ∙ ι)
  redexFresh x#M rewrite sym (substFresh x#M) = beta
